...
class Volume:
    ...
    def is_valid_step(self, step: Optional[Decimal]) -> bool:
        """Checks if volume aligns with the symbol step (e.g. 0.01).

        A missing or zero step means UNSET, so any volume aligns. MT5 exports carry
        VolumeStep 0 on real symbols, and the seeded YAML loses volume_step on a database
        round trip, so a zero reaches here in normal operation. `% 0` raised
        decimal.InvalidOperation, which escaped PreTradeRiskService as an unhandled
        exception instead of becoming a rejection.
        """
        if step is None:
            return True
