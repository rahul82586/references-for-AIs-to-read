...
**The real gap is not "A-Book has no markup mechanism". It is that M7's group spread
is applied only when the matching engine prices a B-Book fill.** An A-Book order is
booked at the LP's raw `AvgPx`, so a group's SpreadDiff earns nothing on hedged flow.
That is debt item 14, and it is the thing to fix.
