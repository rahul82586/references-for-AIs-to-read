...
def _volume_ext(column_value: Any) -> str:
...
def db_to_group(model: GroupModel) -> Group:
    ...
    def _ov(raw: Dict[str, Any], key: str) -> Any:
        # MT5's "default" sentinel means INHERIT the symbol's value. It must
        # come back as None - a sentinel string leaking into the VO would be
        # read as "overridden with something unparseable" and silently zeroed
        # by defensive int() coercion downstream.
        value = raw.get(key)
    ...
    for raw in domain.get("symbol_overrides") or []:
        overrides.append(
            GroupSymbolOverride(
                ...
                trade_mode=_ov(raw, "trade_mode"),
                execution_mode=_ov(raw, "execution_mode"),
                volume_min=_ov(raw, "volume_min"),
                volume_max=_ov(raw, "volume_max"),
                volume_limit=_ov(raw, "volume_limit"),
                spread_diff=_ov(raw, "spread_diff"),
                spread_diff_balance=_ov(raw, "spread_diff_balance"),
                margin_rate_initial_buy=_ov(raw, "margin_rate_initial_buy"),
                margin_rate_initial_sell=_ov(raw, "margin_rate_initial_sell"),
                swap_long=_ov(raw, "swap_long"),
                swap_short=_ov(raw, "swap_short"),
            )
        )
