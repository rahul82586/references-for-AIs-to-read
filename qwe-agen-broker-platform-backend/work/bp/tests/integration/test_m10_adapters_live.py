...
async def test_a_book_order_hedges_over_fix(monkeypatch):
    ...
    h.event_bus.subscribe(EventType.DEAL_CREATED, on_routed)
    ...
    try:
        ...
        # M11: a FILLED report is no longer dropped - it books the client side. This test
        # used to stop at "the LP received a NewOrderSingle", which passed for the whole
        # of M10 while the client got no deal, no position and no margin move. Assert the
        # trade, not just the message.
        a_book_deals = [
            e for e in deal_events
            if e.payload.get("destination") == ExecutionDestination.A_BOOK.value
        ]
        assert a_book_deals, (
            "the LP filled but no A-Book DEAL_CREATED routing event was published"
        )
        payload = a_book_deals[0].payload
        # `hedged` is the M11 statement: a real gateway means the broker passed the
        # risk on. (`stub` lives on the ORDER_ROUTED payload, not this one.)
        assert payload["hedged"] is True
        assert payload["gateway_id"] == "SIMLP"
        assert Decimal(payload["price"]) == Decimal("1.10010")  # simulated LP ask
        assert payload["reconciliation_flags"] == []

        # ...and the client really holds it
        deals = await h.deal_repo.find_by_account(DEFAULT_LOGIN)
        assert len(deals) == 1, "no client deal was booked for the LP's fill"
        assert deals[0].price.value == Decimal("1.10010")
        positions = await h.position_repo.get_by_account(DEFAULT_LOGIN)
        assert len(positions) == 1, "no client position was opened"

        # a real hedge means the broker passed the risk on: net exposure unchanged
