...
async def build_trading_stack(
    ...
    *,
    ...
    elif lp_gateway_kind in ("trade_server", "tradeserver", "mt5"):
        # M11: hedge through a trade-server box with a real MT5 terminal attached -
        # the Centroid Bridge pattern (hedge client flow through MT5 terminals)
        # rather than a bank FIX session.
        from infrastructure.gateways.trade_server_gateway import (
            build_trade_server_gateway_from_env,
        )

        default_gateway = build_trade_server_gateway_from_env()
        logger.info(
            "A-Book liquidity gateway: trade-server MT5 terminal at %s (filling=%s, "
            "timeout=%ss) - hedges are REAL orders on that terminal",
            default_gateway.base_url, default_gateway.type_filling,
            default_gateway.timeout_s,
        )
    elif lp_gateway_kind:
        raise RuntimeError(
            f"unknown BROKER_LP_GATEWAY={lp_gateway_kind!r}; supported: 'fix', "
            "'trade_server' or unset (stub)"
        )
