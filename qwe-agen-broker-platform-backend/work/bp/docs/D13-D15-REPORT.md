...
### It was still happening on the shared database while the fix ran

The proof's own account 886152 ended the run with `balance 100001.0000` and
`equity 100000.5000` — an equity that ignores the +0.50 the balance had just
received. That is the D13 signature: equity written as `balance + the stale
profit`. But the process running the proof had the fix, and its own read back was
correct.

The writer was the **other** server: the one restarted before D13 existed, sharing
the same Neon database, still ticking, still writing `profit = 0` and
`equity = balance` for every account holding one of its positions — 0.5 s after the
proof's own pipeline had written the truth. It also left `margin_free 99923.16`
against `equity 100000.50 / margin_used 0`, and `margin_level 129305.98` where the
no-margin sentinel is `999999`: three writers of derived fields, three sets of
numbers.

So the defect was not hypothetical and not historical — it was corrupting the live
database continuously, and any node running pre-D13 code will keep doing it. **Every
process pointed at a database must run the fixed tree.**
...
## Also observed: derived account fields still have three writers

`margin_free` and `margin_level` diverged from `equity` and `margin_used` on two of
the proof accounts (`99923.16` against equity `100000.50` with margin 0;
`margin_level 129305.98` where the sentinel is `999999`). They are written by
`record_deal._recalculate_account_margin`, by `ClosePositionHandler._revalue_after_close`,
by `account.update_equity()` inside the tick pipeline, and by the valuation sweep —
and they do not all agree about what "no margin" looks like or whether free margin
is `equity - margin_used`. Logged as P2 debt, not fixed here: it needs one owner for
derived account state, which is a design decision rather than a patch.
