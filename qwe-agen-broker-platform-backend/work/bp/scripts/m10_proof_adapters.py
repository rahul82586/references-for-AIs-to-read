...
async def part_b():
    ...
    try:
        ...
        routed: list = []
        h.event_bus.subscribe(EventType.ORDER_ROUTED,
                              lambda e: routed.append(e) if e.event_type == EventType.ORDER_ROUTED else None)
        ...
        executions = [e for e in routed if isinstance(e, DomainEvent) and "execution_report" in e.payload]
        check("B2 A-Book leg executed and emitted OrderRouted", bool(executions))
        report = executions[0].payload["execution_report"] if executions else {}
        check("B3 LP filled the hedge at its ask; report is NOT a stub",
              report.get("status") == "FILLED" and Decimal(report.get("price", "0")) == Decimal("1.10010")
              and executions[0].payload.get("stub") is False)
