...
exports.ImportWizard = void 0;
// @ts-nocheck
const React = require("react");
const ConnectStep_1 = require("./ConnectStep");
const SelectSymbolsStep_1 = require("./SelectSymbolsStep");
const ImportSummary_1 = require("./ImportSummary");
const api_1 = require("../../api");
function ImportWizard({ activeFolder, onClose, onImported }) {
    ...
    const handleConnectChange = (fields) => {
    ...
    const renderStepContent = () => {
        switch (step) {
            case 1:
                return React.createElement(ConnectStep_1.ConnectStep, { data: connectData, onChange: handleConnectChange });
            case 2:
                return (React.createElement(SelectSymbolsStep_1.SelectSymbolsStep, { connectData: connectData, selectedSymbols: selectedSymbols, onSelectSymbolsChange: setSelectedSymbols, overwriteExisting: overwriteExisting, onOverwriteChange: setOverwriteExisting }));
            case 3:
                return React.createElement(ImportSummary_1.ImportSummary, { count: selectedSymbols.length, overwrite: overwriteExisting });
            default:
                return null;
        }
    };
    return (React.createElement("div", { className: "adm-modal-overlay", style: { zIndex: 1100 }, onClick: onClose },
        React.createElement("div", { className: "adm-modal", style: { width: 500 }, onClick: e => e.stopPropagation() },
            React.createElement("div", { className: "adm-modal-header" },
                React.createElement("h2", null, "Import Symbols Configuration Wizard"),
                React.createElement("span", { style: { fontSize: 11, color: 'var(--theia-descriptionForeground)' } },
                    "Step ",
                    step,
                    " of 3")),
            error && (React.createElement("div", { className: "adm-hint", style: { background: 'var(--theia-inputValidation-errorBackground)', color: 'var(--theia-errorForeground)', margin: '10px 16px 0 16px' } },
                React.createElement("i", { className: "codicon codicon-error" }),
                " ",
                error)),
            loading ? (React.createElement("div", { className: "adm-modal-body", style: { height: '30vh', display: 'flex', justifyContent: 'center', alignItems: 'center' } },
                React.createElement("span", null, "Processing steps..."))) : (renderStepContent()),
            React.createElement("div", { className: "adm-modal-footer" },
                step === 3 ? (React.createElement("button", { type: "button", className: "adm-btn adm-btn-primary", onClick: handleDone }, "Done")) : (React.createElement(React.Fragment, null,
                    React.createElement("button", { type: "button", className: "adm-btn", disabled: step === 1 || loading, onClick: handleBack }, "Back"),
                    React.createElement("button", { type: "button", className: "adm-btn adm-btn-primary", disabled: loading, onClick: handleNext }, step === 1 ? 'Connect' : 'Ingest Symbols'))),
